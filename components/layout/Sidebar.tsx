...
import {
  LayoutDashboard, TrendingUp, Bot, FlaskConical, History, Settings, ChevronLeft, ChevronRight, Zap, Brain, Terminal, Flame, X
} from 'lucide-react';
...
export function Sidebar() {
  ...
  const { sidebarOpen, setSidebarOpen, mobileMenuOpen, setMobileMenuOpen } = useStore();
      ...
      {navItems.map(({ href, label, icon: Icon }) => {
        ...
        return (
          <Tooltip key={href}>
            ...
            {!sidebarOpen && !mobileMenuOpen && <TooltipContent side="right">{label}</TooltipContent>}
          </Tooltip>
        );
      })}
    </nav>
    ...
    <TooltipProvider delayDuration={0}>
      ...
      {mobileMenuOpen && (
        <>
          ...
          />
          {/* Drawer */}
          <aside className="xl:hidden fixed inset-y-0 left-0 w-64 z-50 flex flex-col bg-card border-r border-border">
            <div className="flex items-center h-14 px-3 border-b border-border">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center flex-shrink-0">
                  <Zap className="w-4 h-4 text-primary-foreground" />
                </div>
                <span className="font-bold text-sm truncate">CryptoBot</span>
              </div>
